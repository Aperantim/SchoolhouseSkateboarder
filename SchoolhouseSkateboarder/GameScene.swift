...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func resetSkater() {
        // Set the initial position skater zPosition and minimumY
        ...
        skater.zRotation = 0.0 //Свойство zRotation определяет, насколько сильно объект вращается вправо или влево.
        skater.physicsBody?.velocity = CGVector(dx: 0.0, dy: 0.0) //Это заставит ее остановиться во время прыжка или падения.
        skater.physicsBody?.angularVelocity = 0.0 // физическое тело все еще может вращаться, поэтому нужно обнулить и эти значения.
    }
    ...
    func startGame() {
        ...
        //run(SKAction.playSoundFileNamed("skateboard.wav", waitForCompletion: false))
        let action: SKAction = SKAction.playSoundFileNamed("skateboard.wav", waitForCompletion: false)
        self.run(action, withKey:"die-sound")
        ...
        //удаляем все спрайты brick из сцены. В конце игры секции могут оказаться где угодно, поэтому лучше удалить их все
        for brick in bricks {
    ...
    func gameOver() {
        ...
        self.removeAction(forKey: "die-sound")
    ...
    func spawnBrick (atPosition position: CGPoint) -> SKSpriteNode {
        ...
        brickSize = brick.size //? brickSize or bricksSize?
        
        // Добавляем новую секцию к массиву
        bricks.append(brick)
        
        // Настройка физического тела секции
        let center = brick.centerRect.origin //задает центральную точку только что созданного объекта brick
        brick.physicsBody = SKPhysicsBody(rectangleOf: brick.size, center: center) //создает физическое тело и присоединяет его к спрайту brick. Для этого мы создаем прямоугольник того же размера и помещаем его в центре секции. Теперь это физическое тело находится поверх спрай- та brick.
        brick.physicsBody?.affectedByGravity = false //сообщает физическому телу спрайта brick, что на него не должна влиять гравитация: мы же не хотим, чтобы тротуар проваливался!
        brick.physicsBody?.categoryBitMask = PhysicsCategory.brick //сообщает SpriteKit, к како- му типу объекта принадлежит данное тело.
        brick.physicsBody?.collisionBitMask = 0 //сообщает SpriteKit, что секции не долж- ны сталкиваться с чем-либо еще
    ...
    func updateBricks(withScrollAmount currentScrollAmount: CGFloat) {
        ...
        while farthestRightBrickX < frame.width {
            ...
            if randomNumber < 2 && score > 10 {
                ...
                // 2-процентный шанс на то, что у нас
                // возникнет разрыв между секциями когда игрок набрал 10 очков
                ...
                // 4-процентный шанс на изменение уровня секции при достижении игроком 20 очков
                if brickLevel == .high {
                    brickLevel = .low
    ...
    //Будем использовать этот метод для обновления положения скейтбордистки при прыжке
    func updateSkater() {
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        updateSkater()
        scrollSpeed += 0.01 //медленно увеличиваем скорость игры
    ...
    @objc func handleTap(tapGesture: UITapGestureRecognizer) {
        ...
        if gameState == .running {
            ...
            if skater.isOnGround {
                ...
                // импульс к физическому телу спрайта skater
